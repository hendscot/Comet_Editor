...
void
LineManager::InsertLineBefore(LPTR LN) {
	if (NoLines()) {
		Newline();
	}
	else if (LN == strt_) {
...
void
LineManager::InsertLineAfter(LPTR LN) {
...
void
LineManager::InsertChar(const int ln, const int in, char ch) {
	int i;
...
void
LineManager::InsertBreak(const int ln, const int in) {
	int i;
	iter_ = strt_;
	for (i = 0; i < ln; iter_ = iter_->next, i++);
	if (i == ln) {
...
void
LineManager::Append(char ch, const int ln) {
	int i;
	// iterate lines from beginning
	iter_ = strt_;
...
void
LineManager::DeleteChar(const int LN, const int index) {
	iter_ = strt_;
	// find target line
...
void
LineManager::DeleteLine(LPTR LN) {
	if (LN == end_) {
		if (LN->Length() > 0) {
			ConcatLines(LN);
		}
...
/* MAKE THESE STATIC?? */
int
LineManager::GetLength (const int LN) const {
	iter_ = strt_;
	// find target line
	for (int i = 0; i < LN && iter_->next != strt_; i++, iter_ = iter_->next);
	return iter_->Length ();
...
int
LineManager::GetLineCount() const {
	return strt_->lineAmnt;
}

int
LineManager::ConcatLines (LPTR LN) {
	if (LN->prev->size < 80) {
